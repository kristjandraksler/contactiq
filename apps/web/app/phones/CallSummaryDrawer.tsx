...
export default function CallSummaryDrawer({lead,onClose}:{lead:Lead;onClose:()=>void}){
  ...
  return <div className="callDrawerBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><aside className="callDrawer">
    <header className="callDrawerHeader"><div><p className="eyebrow">CALL SUMMARIES</p><h2>{lead.phone??"Telefon"}</h2><p className="muted">{lead.email}</p></div><button onClick={onClose} aria-label="Zapri">×</button></header>
    <form className="callSummaryForm" onSubmit={save}>
      <div className="callFormGrid"><label><span>Call result</span><select value={result} onChange={e=>setResult(e.target.value)}>{Object.entries(RESULTS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label><span>Trajanje (min)</span><input type="number" min="0" value={duration} onChange={e=>setDuration(e.target.value)} placeholder="npr. 8"/></label></div>
      <label><span>Summary</span><textarea required rows={5} value={summary} onChange={e=>setSummary(e.target.value)} placeholder="Kaj se je zgodilo med klicem?"/></label>
      <div className="callFormGrid"><label><span>Next activity</span><select value={action} onChange={e=>setAction(e.target.value)}>{Object.entries(ACTIONS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label><span>Datum in ura</span><input type="datetime-local" value={nextAt} onChange={e=>setNextAt(e.target.value)}/></label></div>
      <label><span>Called</span><input value={createdBy} onChange={e=>setCreatedBy(e.target.value)}/></label>
