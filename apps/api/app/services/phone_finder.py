...
def find_phone_from_pages(
    *,
    ...
    effective_started_at = started_at or time.perf_counter()

    def duration_ms() -> int:
        return int((time.perf_counter() - effective_started_at) * 1000)
    ...
    if not ranked:
        return FinderResult(
            ...
            source_url=None, pages_scanned=len(pages),
            scan_duration_ms=duration_ms(), candidates=[], error=None,
            confidence_label="UNKNOWN",
        )

    best = ranked[0]
    best_confidence = best.confidence if best.confidence is not None else _confidence(best)

    return FinderResult(
        status="MATCHED", website=website, phone=best.phone,
        confidence=best_confidence, source_url=best.source_url,
        pages_scanned=len(pages), scan_duration_ms=duration_ms(),
        candidates=[asdict(candidate) for candidate in ranked[:5]],
        error=None, confidence_label=confidence_label(best_confidence),
    )
...
async def find_phone_for_domain(
    ...
    if not domain or "." not in domain:
        return FinderResult(
            ...
            candidates=[], error="Domena ni veljavna.",
            confidence_label="UNKNOWN",
        )
    ...
    return find_phone_from_pages(
        domain=domain, website=website, pages=pages, started_at=started_at
    )
