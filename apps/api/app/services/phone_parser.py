...
class ExtractedPhone:
    ...
    # Sprint 3.2.1: metadata is collected but not yet used by ranking.
...
def _nearest_keyword(
    ...
    for phrase in KEYWORD_PHRASES:
        ...
        while True:
            ...
            keyword_end = keyword_start + len(phrase)

            if target_start < keyword_start:
                distance = keyword_start - target_start
            elif target_start > keyword_end:
                distance = target_start - keyword_end
            else:
                distance = 0
            ...
            search_from = keyword_start + 1

    return nearest_phrase, nearest_distance
...
def _position_ratio(
    ...
    if start is None or total_length <= 0:
        return None

    ratio = start / total_length
    return round(max(0.0, min(ratio, 1.0)), 4)


def _section_from_context(
    context: str,
    ...
    for section, phrases in section_rules:
...
def extract_phones(
    ...
    full_page_length = len(full_page_text)
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if href.lower().startswith("tel:"):
            ...
            anchor_position = (
                full_page_text.find(anchor_text)
                if anchor_text
                else -1
            )

            keyword, distance = _nearest_keyword(
                context,
                0,
            )
            ...
            _append_candidate(
                ...
                html_tag=anchor.name,
                section=_section_from_context(
                    context,
                    fallback=keyword,
                ),
            )
    ...
    # 3. Microdata and generic metadata.
    selector = (
        '[itemprop="telephone"], '
        'meta[name="telephone"], '
        'meta[property='
    ...
    # Structured signals are read before noisy DOM is removed.
    _remove_noise(soup)

    # Avoid double-counting tel-link text.
    ...
    for footer in list(
        ...
        footer_global_start = (
            full_page_text.find(footer_text)
            if footer_text
            else -1
        )
        ...
        for match in PHONE_PATTERN.finditer(
            ...
            global_start = (
                footer_global_start + start
                if footer_global_start >= 0
                else None
            )
